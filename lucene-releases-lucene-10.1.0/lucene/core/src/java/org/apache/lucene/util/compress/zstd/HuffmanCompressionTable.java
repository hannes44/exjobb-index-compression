...
final class HuffmanCompressionTable
{
    ...
    public static int optimalNumberOfBits(int maxNumberOfBits, int inputSize, int maxSymbol)
    {
        ...
        result = Math.min(result, highestBit((inputSize - 1)) - 1); // we may be able to reduce accuracy if input is small
    ...
    public int write(DataOutput outputBase, long outputAddress, int outputSize, HuffmanTableWriterWorkspace workspace) throws IOException {
        ...
        ByteBuffersDataOutput compressedWeights = new ByteBuffersDataOutput();
        int size = compressWeights(compressedWeights, output + 1, outputSize - 1, weights, maxSymbol, workspace);
        ...
        if (size != 0 && size != 1 && size < maxSymbol / 2) {
            ...
            outputBase.writeByte((byte) size);
            compressedWeights.copyTo(outputBase);
            ...
            // REDUNDANT ?? int entryCount = maxSymbol;

            size = (maxSymbol + 1) / 2;  // ceil(#entries / 2)
    ...
    private static int setMaxHeight(NodeTable nodeTable, int lastNonZero, int maxNumberOfBits, HuffmanCompressionTableWorkspace workspace)
    {
        ...
        while (totalCost > 0) {
            int numberOfBitsToDecrease = highestBit(totalCost) + 1;
     ...
     */
    private static int compressWeights(DataOutput outputBase, long outputAddress, int outputSize, byte[] weights, int weightsLength, HuffmanTableWriterWorkspace workspace) throws IOException {
        if (weightsLength <= 1) {
