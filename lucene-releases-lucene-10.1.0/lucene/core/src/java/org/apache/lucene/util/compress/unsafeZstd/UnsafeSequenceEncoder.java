...
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeConstants.DEFAULT_MAX_OFFSET_CODE_SYMBOL;
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeConstants.LITERALS_LENGTH_BITS;
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeConstants.LITERAL_LENGTH_TABLE_LOG;
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeConstants.LONG_NUMBER_OF_SEQUENCES;
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeConstants.MATCH_LENGTH_BITS;
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeConstants.MATCH_LENGTH_TABLE_LOG;
...
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeConstants.OFFSET_TABLE_LOG;
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeConstants.SEQUENCE_ENCODING_BASIC;
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeConstants.SEQUENCE_ENCODING_COMPRESSED;
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeConstants.SEQUENCE_ENCODING_RLE;
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeConstants.SIZE_OF_SHORT;
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeFiniteStateEntropy.optimalTableLog;
...
final class UnsafeSequenceEncoder
{
    ...
    private static final UnsafeFseCompressionTable DEFAULT_LITERAL_LENGTHS_TABLE = UnsafeFseCompressionTable.newInstance(DEFAULT_LITERAL_LENGTH_NORMALIZED_COUNTS, MAX_LITERALS_LENGTH_SYMBOL, DEFAULT_LITERAL_LENGTH_NORMALIZED_COUNTS_LOG);
    private static final UnsafeFseCompressionTable DEFAULT_MATCH_LENGTHS_TABLE = UnsafeFseCompressionTable.newInstance(DEFAULT_MATCH_LENGTH_NORMALIZED_COUNTS, MAX_MATCH_LENGTH_SYMBOL, DEFAULT_LITERAL_LENGTH_NORMALIZED_COUNTS_LOG);
    private static final UnsafeFseCompressionTable DEFAULT_OFFSETS_TABLE = UnsafeFseCompressionTable.newInstance(DEFAULT_OFFSET_NORMALIZED_COUNTS, DEFAULT_MAX_OFFSET_CODE_SYMBOL, DEFAULT_OFFSET_NORMALIZED_COUNTS_LOG);

    private UnsafeSequenceEncoder()
    {
    }

    public static int compressSequences(Object outputBase, final long outputAddress, int outputSize, UnsafeSequenceStore sequences, UnsafeCompressionParameters.Strategy strategy, UnsafeSequenceEncodingContext workspace)
    {
        ...
        UnsafeHistogram.count(sequences.literalLengthCodes, sequenceCount, workspace.counts);
        maxSymbol = UnsafeHistogram.findMaxSymbol(counts, MAX_LITERALS_LENGTH_SYMBOL);
        largestCount = UnsafeHistogram.findLargestCount(counts, maxSymbol);
        ...
        UnsafeFseCompressionTable literalLengthTable;
        ...
        // offsets
        UnsafeHistogram.count(sequences.offsetCodes, sequenceCount, workspace.counts);
        maxSymbol = UnsafeHistogram.findMaxSymbol(counts, MAX_OFFSET_CODE_SYMBOL);
        largestCount = UnsafeHistogram.findLargestCount(counts, maxSymbol);
        ...
        UnsafeFseCompressionTable offsetCodeTable;
        ...
        // match lengths
        UnsafeHistogram.count(sequences.matchLengthCodes, sequenceCount, workspace.counts);
        maxSymbol = UnsafeHistogram.findMaxSymbol(counts, MAX_MATCH_LENGTH_SYMBOL);
        largestCount = UnsafeHistogram.findLargestCount(counts, maxSymbol);
        ...
        UnsafeFseCompressionTable matchLengthTable;
    ...
    private static int buildCompressionTable(UnsafeFseCompressionTable table, Object outputBase, long output, long outputLimit, int sequenceCount, int maxTableLog, byte[] codes, int[] counts, int maxSymbol, short[] normalizedCounts)
    {
        ...
        UnsafeFiniteStateEntropy.normalizeCounts(normalizedCounts, tableLog, counts, sequenceCount, maxSymbol);
        table.initialize(normalizedCounts, maxSymbol, tableLog);

        return UnsafeFiniteStateEntropy.writeNormalizedCounts(outputBase, output, (int) (outputLimit - output), normalizedCounts, maxSymbol, tableLog); // TODO: pass outputLimit directly
    }
    ...
    private static int encodeSequences(
            ...
            long outputLimit,
            UnsafeFseCompressionTable matchLengthTable,
            UnsafeFseCompressionTable offsetsTable,
            UnsafeFseCompressionTable literalLengthTable,
            UnsafeSequenceStore sequences)
    {
        ...
        UnsafeBitOutputStream blockStream = new UnsafeBitOutputStream(outputBase, output, (int) (outputLimit - output));
    ...
    private static int selectEncodingType(
            ...
            UnsafeCompressionParameters.Strategy strategy)
    {
        ...
        if (strategy.ordinal() < UnsafeCompressionParameters.Strategy.LAZY.ordinal()) { // TODO: more robust check. Maybe encapsulate in strategy objects
