...
final class UnsafeFiniteStateEntropy
{
    ...
    private UnsafeFiniteStateEntropy()
    {
    }

    public static int decompress(UnsafeFiniteStateEntropy.Table table, final Object inputBase, final long inputAddress, final long inputLimit, byte[] outputBuffer)
    {
        ...
        loader = new UnsafeBitInputStream.Loader(inputBase, input, currentAddress, bits, bitsConsumed);
        loader.load();
        bits = loader.getBits();
        bitsConsumed = loader.getBitsConsumed();
        currentAddress = loader.getCurrentAddress();
    ...
    public static int compress(Object outputBase, long outputAddress, int outputSize, byte[] input, int inputSize, UnsafeFseCompressionTable table)
    {
        return compress(outputBase, outputAddress, outputSize, input, ARRAY_BYTE_BASE_OFFSET, inputSize, table);
    }

    public static int compress(Object outputBase, long outputAddress, int outputSize, Object inputBase, long inputAddress, int inputSize, UnsafeFseCompressionTable table)
    {
        ...
        UnsafeBitOutputStream stream = new UnsafeBitOutputStream(outputBase, outputAddress, outputSize);
    ...
    public static int optimalTableLog(int maxTableLog, int inputSize, int maxSymbol)
    {
        ...
        result = Math.min(result, UnsafeZSTDUtil.highestBit((inputSize - 1)) - 2); // we may be able to reduce accuracy if input is small

        // Need a minimum to safely represent all symbol values
        result = Math.max(result, UnsafeZSTDUtil.minTableLog(inputSize, maxSymbol));
    ...
    public static int normalizeCounts(short[] normalizedCounts, int tableLog, int[] counts, int total, int maxSymbol)
    {
        ...
        checkArgument(tableLog >= UnsafeZSTDUtil.minTableLog(total, maxSymbol), "FSE table size too small");
