...
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeZSTDUtil.get24BitLittleEndian;
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeZSTDUtil.verify;
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeZSTDUtil.fail;
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeZSTDUtil.mask;
import static sun.misc.Unsafe.ARRAY_BYTE_BASE_OFFSET;
/**
 * Zstandard compression and decompression routines.
 */
public final class UnsafeZSTD {
    ...
    private static final UnsafeFiniteStateEntropy.Table literalsLengthTable = new UnsafeFiniteStateEntropy.Table(LITERAL_LENGTH_TABLE_LOG);
    private static final UnsafeFiniteStateEntropy.Table offsetCodesTable = new UnsafeFiniteStateEntropy.Table(OFFSET_TABLE_LOG);
    private static final UnsafeFiniteStateEntropy.Table matchLengthTable = new UnsafeFiniteStateEntropy.Table(MATCH_LENGTH_TABLE_LOG);

    private static UnsafeFiniteStateEntropy.Table currentLiteralsLengthTable;
    private static UnsafeFiniteStateEntropy.Table currentOffsetCodesTable;
    private static UnsafeFiniteStateEntropy.Table currentMatchLengthTable;

    private static final UnsafeHuffman UNSAFE_HUFFMAN = new UnsafeHuffman();
    private static final UnsafeFseTableReader fse = new UnsafeFseTableReader();
