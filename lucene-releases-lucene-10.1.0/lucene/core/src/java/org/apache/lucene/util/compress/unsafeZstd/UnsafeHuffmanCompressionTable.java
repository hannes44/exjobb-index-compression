...
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeHuffman.MIN_TABLE_LOG;
...
import static org.apache.lucene.util.compress.unsafeZstd.UnsafeZSTDUtil.minTableLog;

final class UnsafeHuffmanCompressionTable
{
    ...
    public UnsafeHuffmanCompressionTable(int capacity)
    {
    ...
    public static int optimalNumberOfBits(int maxNumberOfBits, int inputSize, int maxSymbol)
    {
        ...
        result = Math.min(result, UnsafeZSTDUtil.highestBit((inputSize - 1)) - 1); // we may be able to reduce accuracy if input is small
    ...
    public void initialize(int[] counts, int maxSymbol, int maxNumberOfBits, UnsafeHuffmanCompressionTableWorkspace workspace)
    {
        checkArgument(maxSymbol <= MAX_SYMBOL, "Max symbol value too large");

        workspace.reset();

        UnsafeNodeTable unsafeNodeTable = workspace.unsafeNodeTable;
        unsafeNodeTable.reset();

        int lastNonZero = buildTree(counts, maxSymbol, unsafeNodeTable);

        // enforce max table log
        maxNumberOfBits = setMaxHeight(unsafeNodeTable, lastNonZero, maxNumberOfBits, workspace);
        ...
        for (int node = 0; node < symbolCount; node++) {
            int symbol = unsafeNodeTable.symbols[node];
            numberOfBits[symbol] = unsafeNodeTable.numberOfBits[node];
        }
        ...
        for (int n = 0; n <= lastNonZero; n++) {
            entriesPerRank[unsafeNodeTable.numberOfBits[n]]++;
        }
    ...
    private int buildTree(int[] counts, int maxSymbol, UnsafeNodeTable unsafeNodeTable)
    {
        ...
        for (int symbol = 0; symbol <= maxSymbol; symbol++) {
            ...
            while (position > 1 && count > unsafeNodeTable.count[position - 1]) {
                unsafeNodeTable.copyNode(position - 1, position);
                position--;
            }

            unsafeNodeTable.count[position] = count;
            unsafeNodeTable.symbols[position] = symbol;

            current++;
        }

        int lastNonZero = maxSymbol;
        while (unsafeNodeTable.count[lastNonZero] == 0) {
            lastNonZero--;
        }
        ...
        unsafeNodeTable.count[current] = unsafeNodeTable.count[currentLeaf] + unsafeNodeTable.count[currentLeaf - 1];
        unsafeNodeTable.parents[currentLeaf] = current;
        unsafeNodeTable.parents[currentLeaf - 1] = current;
        current++;
        currentLeaf -= 2;
        ...
        for (int n = current; n <= root; n++) {
            unsafeNodeTable.count[n] = 1 << 30;
        }

        // create parents
        while (current <= root) {
            int child1;
            if (currentLeaf >= 0 && unsafeNodeTable.count[currentLeaf] < unsafeNodeTable.count[currentNonLeaf]) {
                child1 = currentLeaf--;
            }
            else {
            ...
            unsafeNodeTable.count[current] = unsafeNodeTable.count[child1] + unsafeNodeTable.count[child2];
            unsafeNodeTable.parents[child1] = current;
            unsafeNodeTable.parents[child2] = current;
            current++;
        }

        // distribute weights
        unsafeNodeTable.numberOfBits[root] = 0;
        for (int n = root - 1; n >= nonLeafStart; n--) {
            short parent = unsafeNodeTable.parents[n];
            unsafeNodeTable.numberOfBits[n] = (byte) (unsafeNodeTable.numberOfBits[parent] + 1);
        }
    ...
    public void encodeSymbol(UnsafeBitOutputStream output, int symbol)
    {
        output.addBitsFast(values[symbol], numberOfBits[symbol]);
    }

    public int write(Object outputBase, long outputAddress, int outputSize, UnsafeHuffmanTableWriterWorkspace workspace)
    {
    ...
    private static int setMaxHeight(UnsafeNodeTable unsafeNodeTable, int lastNonZero, int maxNumberOfBits, UnsafeHuffmanCompressionTableWorkspace workspace)
    {
        int largestBits = unsafeNodeTable.numberOfBits[lastNonZero];
        ...
        while (unsafeNodeTable.numberOfBits[n] > maxNumberOfBits) {
            totalCost += baseCost - (1 << (largestBits - unsafeNodeTable.numberOfBits[n]));
            unsafeNodeTable.numberOfBits[n ] = (byte) maxNumberOfBits;
            n--;
        ...
        for (int pos = n; pos >= 0; pos--) {
            if (unsafeNodeTable.numberOfBits[pos] >= currentNbBits) {
                continue;
            }
            currentNbBits = unsafeNodeTable.numberOfBits[pos];   // < maxNumberOfBits
            rankLast[maxNumberOfBits - currentNbBits] = pos;
        }

        while (totalCost > 0) {
            int numberOfBitsToDecrease = UnsafeZSTDUtil.highestBit(totalCost) + 1;
            for (; numberOfBitsToDecrease > 1; numberOfBitsToDecrease--) {
                ...
                int highTotal = unsafeNodeTable.count[highPosition];
                int lowTotal = 2 * unsafeNodeTable.count[lowPosition];
            ...
            unsafeNodeTable.numberOfBits[rankLast[numberOfBitsToDecrease]]++;
            if (rankLast[numberOfBitsToDecrease] == 0) {   /* special case, reached largest symbol */
                ...
                if (unsafeNodeTable.numberOfBits[rankLast[numberOfBitsToDecrease]] != maxNumberOfBits - numberOfBitsToDecrease) {
        ...
        while (totalCost < 0) {  // Sometimes, cost correction overshoot
            if (rankLast[1] == noSymbol) {  /* special case : no rank 1 symbol (using maxNumberOfBits-1); let's create one from largest rank 0 (using maxNumberOfBits) */
                ...
                unsafeNodeTable.numberOfBits[n + 1]--;
                rankLast[1] = n + 1;
                totalCost++;
                continue;
            }
            unsafeNodeTable.numberOfBits[rankLast[1] + 1]--;
            rankLast[1]++;
            totalCost++;
        }
     ...
     */
    private static int compressWeights(Object outputBase, long outputAddress, int outputSize, byte[] weights, int weightsLength, UnsafeHuffmanTableWriterWorkspace workspace)
    {
        if (weightsLength <= 1) {
        ...
        UnsafeHistogram.count(weights, weightsLength, counts);
        int maxSymbol = UnsafeHistogram.findMaxSymbol(counts, MAX_TABLE_LOG);
        int maxCount = UnsafeHistogram.findLargestCount(counts, maxSymbol);
        ...
        int tableLog = UnsafeFiniteStateEntropy.optimalTableLog(MAX_FSE_TABLE_LOG, weightsLength, maxSymbol);
        UnsafeFiniteStateEntropy.normalizeCounts(normalizedCounts, tableLog, counts, weightsLength, maxSymbol);

        long output = outputAddress;
        long outputLimit = outputAddress + outputSize;

        // Write table description header
        int headerSize = UnsafeFiniteStateEntropy.writeNormalizedCounts(outputBase, output, outputSize, normalizedCounts, maxSymbol, tableLog);
        output += headerSize;

        // Compress
        UnsafeFseCompressionTable compressionTable = workspace.fseTable;
        compressionTable.initialize(normalizedCounts, maxSymbol, tableLog);
        int compressedSize = UnsafeFiniteStateEntropy.compress(outputBase, output, (int) (outputLimit - output), weights, weightsLength, compressionTable);
        if (compressedSize == 0) {
            return 0;
        }
