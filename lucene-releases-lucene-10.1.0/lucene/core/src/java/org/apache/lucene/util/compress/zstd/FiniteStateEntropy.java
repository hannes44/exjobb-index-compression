...
import static org.apache.lucene.util.compress.zstd.ZSTDUtil.*;

public final class FiniteStateEntropy
{
    ...
    public static int decompress(FiniteStateEntropy.Table table, final int inputAddress, final long inputLimit, byte[] outputBuffer, byte[] originalInput) throws IOException {
        //final byte[] outputBase = outputBuffer;
        final long outputLimit = + outputBuffer.length;
        ...
        loader = new BitInputStream.Loader(originalInput, inputAddress, currentAddress, bits, bitsConsumed);
        loader.load();
        bits = loader.getBits();
        bitsConsumed = loader.getBitsConsumed();
        currentAddress = loader.getCurrentAddress();
        ...
        while (true) {
            verify(output <= outputLimit - 2, inputAddress, "Output buffer is too small");
            outputBuffer[output++] = symbols[state1];
    ...
    public static int compress(DataOutput outputBase, long outputAddress, int outputSize, byte[] inputBase, int inputSize, FseCompressionTable table) throws IOException {
        checkArgument(outputSize >= SIZE_OF_LONG, "Output buffer too small");

        final long start = 0;
        final long inputLimit = start + inputSize;

        int input = (int) inputLimit;
    ...
    public static int optimalTableLog(int maxTableLog, int inputSize, int maxSymbol)
    {
        ...
        result = Math.min(result, highestBit((inputSize - 1)) - 2); // we may be able to reduce accuracy if input is small
    ...
    public static int normalizeCounts(short[] normalizedCounts, int tableLog, int[] counts, int total, int maxSymbol)
    {
        ...
        checkArgument(tableLog >= minTableLog(total, maxSymbol), "FSE table size too small");
    ...
    public static int writeNormalizedCounts(DataOutput outputBase, long outputAddress, int outputSize, short[] normalizedCounts, int maxSymbol, int tableLog) throws IOException {
        ...
        while (remaining > 1) {
            if (previousIs0) {
                ...
                while (symbol >= start + 24) {
                    ...
                    checkArgument(output + SIZE_OF_SHORT <= outputLimit, "Output buffer too small"); // USE?

                    outputBase.writeShort((short) bitStream);
