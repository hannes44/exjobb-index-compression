...
final class UnsafeSequenceStore
{
    ...
    public UnsafeSequenceStore(int blockSize, int maxSequences)
    {
    ...
    public void generateCodes()
    {
        for (int i = 0; i < sequenceCount; ++i) {
            ...
            offsetCodes[i] = (byte) UnsafeZSTDUtil.highestBit(offsets[i]);
            matchLengthCodes[i] = (byte) matchLengthToCode(matchLengths[i]);
        }

        if (longLengthField == LongField.LITERAL) {
            literalLengthCodes[longLengthPosition] = UnsafeConstants.MAX_LITERALS_LENGTH_SYMBOL;
        }
        if (longLengthField == LongField.MATCH) {
            matchLengthCodes[longLengthPosition] = UnsafeConstants.MAX_MATCH_LENGTH_SYMBOL;
        }
    }
    ...
    {
        if (literalLength >= 64) {
            return UnsafeZSTDUtil.highestBit(literalLength) + 19;
        }
        else {
    ...
    private static int matchLengthToCode(int matchLengthBase)
    {
        if (matchLengthBase >= 128) {
            return UnsafeZSTDUtil.highestBit(matchLengthBase) + 36;
        }
        else {
